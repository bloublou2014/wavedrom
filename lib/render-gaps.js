...
function renderGapUses(text, lane) {
    ...
    while (Stack.length) {
        ...
        if (next === '|') {
            // translate
            const translateX = lane.xs * ((pos - (subCycle ? 0 : lane.period)) * lane.hscale - lane.phase);
            res.push(['use', {'xlink:href': '#gap', transform: 'translate(' + translateX + ')'}]);
        }
    }
    return res;
}
