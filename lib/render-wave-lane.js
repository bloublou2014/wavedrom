...
const findLaneMarkers = require('./find-lane-markers');

function renderLaneUses(cont, lane, color) {
    let i, k;
    const res = [];
    let labels = [];
    if (cont[1]) {
...
function renderWaveLane(content, index, lane, colors) {
    let j,
        name,
        xoffset,
        xmax = 0;
    ...
    for (j = 0; j < content.length; j += 1) {
        ...
        if (name) { // check name
            ...
            res.push([
                ...
                }].concat(renderLaneUses(content[j], lane, color))
            ]);
