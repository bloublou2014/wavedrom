...
function laneParamsFromSkin(index, source) {
    ...
    return _.defaults({
        xs: Number(socket.width),
        ys: Number(socket.height),
        xlabel: Number(socket.x),
        ym: Number(socket.y)
    }, defaultLane);
}
...
function renderSignal(index, source) {
    ...
    const waveColors = _.map(source.signal, 'color');
    const waveLanes = renderWaveLane(content, index, lane, waveColors);
